...
function ENT:SpawnFunction(client, trace)
	...
	local entity = ents.Create("nut_unionlock")
...
if (SERVER) then
	...
	function ENT:Use(activator)
		...
		if (!activator:IsCombine() or activator:Team() != FACTION_ADMIN) or activator:Team() != FACTION_CIVILWORKER) then
			self:Error()
	...
	function ENT:Error()
		...
		timer.Create("nut_UnionLockErroring"..self:EntIndex(), 1, 2, function()
